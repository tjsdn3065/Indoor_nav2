"""Go2 + small_house bringup using the Voronoi global planner with AMCL.

Mirrors `bringup_go2_amcl.launch.py` exactly except for the default
`params_file`, which points to `nav2_params_go2_voronoi.yaml`. That params
file swaps `nav2_navfn_planner::NavfnPlanner` for
`nav2_voronoi_planner::VoronoiPlanner` (using the Go2-tuned roadmap
`small_house_roadmap_go2.json`) and tightens inflation to match.
"""
...
def generate_launch_description():
    ...
    default_params = str(pkg_share / 'config' / 'nav2_params_go2_voronoi.yaml')
    ...
    declare_params = DeclareLaunchArgument(
        ...
        description='Nav2 parameters YAML (Go2 + AMCL + Voronoi profile).',
    )
