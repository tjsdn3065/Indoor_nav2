...
def generate_launch_description():
    ...
    mapping_yaml = str(pkg_share / 'config' / 'rtabmap_mapping.yaml')
    p2l_yaml = str(pkg_share / 'config' / 'pointcloud_to_laserscan.yaml')
    default_db_path = str(Path.home() / '.ros' / 'rtabmap.db')
    ...
    declare_database_path = DeclareLaunchArgument(
        'database_path', default_value=default_db_path,
        description='RTAB-Map database file path. Must be non-empty — '
                    'an empty value makes RTAB-Map fall back to an '
                    'in-memory DB that is discarded on shutdown.',
    )
    declare_delete_db = DeclareLaunchArgument(
        'delete_db_on_start', default_value='false',
        description='Wipe any existing DB at database_path before mapping (start fresh).',
    )
    ...
    rgbd_sync = Node(
        ...
        namespace='rtabmap',
        output='screen',
        parameters=[common_yaml, mapping_yaml, common_overrides],
    ...
    delete_db_arg = PythonExpression([
        '"-d" if "', delete_db_on_start, '" == "true" else ""',
    ])
